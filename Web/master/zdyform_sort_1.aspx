<% @ Page Language="C#" Inherits="PageAdmin.zdyform_sort_1" %>
...
<tr>
  <td  colspan=2 height=25><b>大类设置</b></td>
 </tr>
</table>
             ...
             <ItemTemplate>
                ...
                <tr>
                 ...
                 <asp:Label id="Lbid" Text='<%#DataBinder.Eval(Container.DataItem,"id")%>' runat="server" visible="false"/>
                   <asp:HyperLink id="Edit" runat="server" Text="小类设置" NavigateUrl='<%#"zdyform_sort_2.aspx?fid="+DataBinder.Eval(Container.DataItem,"fid")+"&ftable="+Request.QueryString["ftable"]+"&sort1_id="+DataBinder.Eval(Container.DataItem,"id")+"&sort1_name="+Server.UrlEncode(DataBinder.Eval(Container.DataItem,"sort_name").ToString())%>'/>
                   <asp:LinkButton id="Delete" runat="server" Text="删除"  CommandName="Delete" />
                   <a href="memberpublish_set.aspx?id=<%#DataBinder.Eval(Container.DataItem,"id")%>&table=zdyform_sort&type1=<%#Server.UrlEncode(DataBinder.Eval(Container.DataItem,"sort_name").ToString())%>&backurl=<%#Server.UrlEncode(Request.RawUrl)%>">会员发布设置</a>
                  </td>
                </tr>
              </table>
             </ItemTemplate>

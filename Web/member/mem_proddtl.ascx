<% @  Control Language="c#" Inherits="PageAdmin.mem_proddtl" %>
<div class="uc_lanmu_box">
<div class="uc_lanmu_site">
<ul>
<li class="uc_lanmu_site_1">当前位置：首页 &gt; <a href="/member/index.aspx?language=<%=Request.QueryString["language"]%>">会员中心</a> &gt; 产品编辑</li>
<li class="uc_lanmu_site_2">产品编辑</li>
</ul>
</div>
...
<input type="hidden" id="Sort_1_text" name="Sort_1_text" value="<%=Sort1%>"><input type="hidden" id="Sort_2_text" name="Sort_2_text" value="<%=Sort2%>">
      ...
      <tr>
          ...
          <td><input type="text" name="TBname" id="TBname" Maxlength=50 size=40 class="tb" value="<%=Name%>"></td>
      </tr>
      <tr>
          <td  height=25 >产品型号</td>
          <td ><input type="text" name="TBxinghao" Maxlength=40 size=40 class="tb" value="<%=Xinghao%>"></td>
      </tr>
</tr>  
     <tr>
          <td  height=25 >产品价格</td>
          <td ><input type="text" name="TBprice"   Maxlength=15 size=10  onkeyup="if(isNaN(value))execCommand('undo')" class="tb" value="<%=Price%>"> 单位：<input type="text" name="TBunit"  Maxlength=15 size=6 value="<%=Unit%>" class="tb">
     ...
     <ul title="<%#DataBinder.Eval(Container.DataItem,"m_type")%>价格">
      <li>
         ...
         <input type="text" name="TBMprice"     Maxlength=15 size=10  onkeyup="if(isNaN(value))execCommand('undo')" class="tb" value="<%#Get_Price(DataBinder.Eval(Container.DataItem,"id").ToString())%>")> <%#DataBinder.Eval(Container.DataItem,"m_type")%>
     </li>
     </ul>
     </ItemTemplate>
     </asp:Repeater>
...
<tr>
     ...
     <tr>
          ...
          <td>
              <input type="file"  size=40  name="Upload" id="Upload" class="tb" > <%=Upload_View%>
      ...
      <tr>
          ...
          <td><input  type="text" name="TBkeywords" Maxlength="100"  size=60 class="tb" value="<%=Keywords%>">
        </td>
      </tr>
      <tr>
          <td height=25 >seo描述</td>
          <td><textarea name="TBdescription" Cols="55"  rows="5" onkeyup="if(this.value.length>250){this.value=this.value.substring(0,250)}" class="tb"><%=Description%></textarea>
        </td>
      </tr>
     ...
     <tr>
          <td>产品简介</td>
          <td>
            <textarea name="Introduct" id="Introduct"><%=Introduct%></textarea>
...
<input type="submit" value=" 提 交 "     class="bt" onclick="return Check_Product_Form(0)" style="display:<%=Edit_Show%>">
